'''
This code trains the IACN model for the given dataset.
The task is: interaction prediction.

How to run:
$ python IACN.py --network reddit --epochs 50
'''

from library_data import *
import IACN_models as lib
from IACN_models import *
from sklearn.cluster import KMeans

# INITIALIZE PARAMETERS
parser = argparse.ArgumentParser()
parser.add_argument('--network', default = 'reddit2', help='Name of the network/dataset')
...
parser.add_argument('--week', default=1, type=int, help='Number of weeks in training set')
...
[user2id, user_sequence_id, user_timediffs_sequence, user_previous_itemid_sequence,
 ...
 timestamp_sequence, feature_sequence, y_true, index] = load_network(args)
...
'''
Timespan is the frequency at which the batches are created and the IACN model is trained.
...
The batches are then used to train IACN.
...
tbatch_timespan = timespan / 1500

# INITIALIZE MODEL AND PARAMETERS
model = IACN(args, num_features, num_users, num_items).cuda()
weight = torch.Tensor([1,true_labels_ratio]).cuda()
crossEntropyLoss = nn.CrossEntropyLoss(weight=weight)
MSELoss = nn.MSELoss()

# INITIALIZE EMBEDDING
initial_user_embedding = nn.Parameter(F.normalize(torch.rand(2*args.embedding_dim).cuda(), dim=0)) # the initial user and item embeddings are learned during training as well
initial_item_embedding = nn.Parameter(F.normalize(torch.rand(args.embedding_dim).cuda(), dim=0))
initial_local_embedding= nn.Parameter(torch.zeros(num_users).cuda())
...
local_embedding = initial_local_embedding.repeat(num_users,1)

inital_alpha = nn.Parameter(F.normalize(torch.rand(num_users).cuda(), dim=0))
inital_delta = nn.Parameter(F.normalize(torch.rand(num_users).cuda(), dim=0))
...
alpha = inital_alpha.repeat(num_users,1)
delta = inital_delta.repeat(num_users,1)
...
user_current_timestamp = torch.zeros(num_users,1).cuda()
user_current_transaction_ids = torch.zeros(num_users,num_users).cuda()
user_previous_itemid = torch.ones([num_users], dtype=torch.long).cuda()
existing_weights = torch.zeros([num_users, num_users]).cuda()

# RUN THE IACN MODEL
'''
THE MODEL IS TRAINED FOR SEVERAL EPOCHS. IN EACH EPOCH, IACN USES THE TRAINING SET OF INTERACTIONS TO UPDATE ITS PARAMETERS.
'''
print ("*** Training the IACN model for %d epochs ***" % args.epochs)

item_user = torch.zeros([num_items+1,num_users]).cuda()
    ...
    for ep in progress_bar1:
        ...
        with trange(train_end_idx) as progress_bar2:
            for j in progress_bar2:
                ...
                lib.current_tbatches_timestamps[tbatch_to_insert].append(timestamp_sequence[j])
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    for i in range(len(lib.current_tbatches_user)):
                        ...
                        for cnt,t in enumerate(lib.current_tbatches_timestamps[i] ):
                            user_current_timestamp[lib.current_tbatches_user[i][cnt]]= t
                        ...
                        timestamp_tensor = torch.Tensor(lib.current_tbatches_timestamps[i]).cuda()
                        ...
                        #local_user_embedding_input = local_user_embedding[tbatch_userids,:]
                        user_projected_embedding = model.project_user(args,user_embedding_input, tbatch_userids,local_embedding, timediffs=user_timediffs_tensor,timestamps = timestamp_tensor, user_timestamp = user_current_timestamp, user_embeddings=user_embeddings,
                                                                      user_current_transaction_ids = user_current_transaction_ids, alpha =alpha, delta = delta)
                        ...
                        item_user_embedding = torch.cat([item_embedding_input, item_embedding_static[tbatch_itemids,:], user_projected_embedding, user_embedding_static[tbatch_userids,:]], dim=1)
                        # PREDICT NEXT ITEM EMBEDDING
                        predicted_item_embedding = model.predict_item_embedding(user_item_embedding)
                        # CALCULATE PREDICTION LOSS

                        loss += MSELoss(predicted_item_embedding, torch.cat([item_embedding_input, item_embedding_static[tbatch_itemids,:]], dim=1))

                        # UPDATE DYNAMIC EMBEDDINGS AFTER INTERACTION
                        user_embedding_output = model.forward(args, user_embedding_input, item_embedding_input, timediffs=user_timediffs_tensor, features=feature_tensor, select='user_update')
                        item_embedding_output = model.forward(args, user_embedding_input, item_embedding_input, timediffs=item_timediffs_tensor, features=feature_tensor, select='item_update')
                        local_embedding = model.compute_NN(tbatch_itemids, tbatch_userids, local_embedding, item_user)
                        ...
                        user_embeddings_timeseries[tbatch_interactionids,:] = user_embedding_output[:,:args.embedding_dim]
                        ...
                        item_user[tbatch_itemids, tbatch_userids] = 1


                        user_current_transaction_ids[:, tbatch_userids] +=  1
                        user_current_transaction_ids[tbatch_userids, :] =0
                    ...
                    timestamp_tensor.detach()
                    local_embedding.detach_()
        ...
        save_model(model, optimizer, args, ep, user_embeddings_dystat, item_embeddings_dystat, local_embedding, train_end_idx,alpha, delta, item_user, user_current_timestamp,user_current_transaction_ids,user_embeddings_timeseries, item_embeddings_timeseries)
