'''
This code evaluates the validation and test performance in an epoch of the model trained in IACN.py.
...
$ python evaluate_interaction_prediction.py --network reddit  --epoch 49
...
parser.add_argument('--network', default='reddit2', help='Network name')
...
parser.add_argument('--embedding_dim', default=64, type=int, help='Number of dimensions')
...
[user2id, user_sequence_id, user_timediffs_sequence, user_previous_itemid_sequence, \
 ...
 timestamp_sequence, \
 feature_sequence, \
 y_true, index] = load_network(args)
...
# LOAD THE MODEL
model, optimizer, user_embeddings_dystat, item_embeddings_dystat, local_embedding,user_embeddings_timeseries, item_embeddings_timeseries,user_current_transactionids, train_end_idx_training, alpha, delta,item_user, user_current_timestamp = load_model(model, optimizer, args, args.epoch)
...
user_embeddings = user_embeddings_dystat[:, :2*args.embedding_dim]
user_embeddings = user_embeddings.clone()
user_embeddings_static = user_embeddings_dystat[:, 2*args.embedding_dim:]
...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        timestamp_tensor = Variable(torch.Tensor([timestamp_sequence[j]]).cuda())
        ...
        itemid_tensor= torch.cuda.LongTensor([itemid])
        userid_tensor = torch.cuda.LongTensor([userid])
        ...
        user_projected_embedding = model.project_user(args,user_embedding_input, userid_tensor, local_embedding, timediffs=user_timediffs_tensor,timestamps = timestamp_tensor, user_timestamp = user_current_timestamp, user_embeddings=user_embeddings,
                                                                         user_current_transaction_ids =user_current_transactionids , alpha =alpha, delta = delta)
        ...
        item_embedding_clone = item_embeddings.clone()
        #item_user_embeddings = torch.cat([item_embeddings, item_embeddings_static, user_projected_embedding.repeat(num_items, 1), user_embedding_static_input.repeat(num_items,1)], dim=1)
        # PREDICT ITEM EMBEDDING
        predicted_item_embedding = model.predict_item_embedding(user_item_embedding)

        actual_item_embedding_cat = torch.cat([item_embeddings,item_embeddings_static],dim=1)
        ...
        euclidean_distances = nn.PairwiseDistance()(predicted_item_embedding.repeat(num_items, 1), actual_item_embedding_cat).squeeze(-1)
        ...
        user_embedding_output = model.forward(args,user_embedding_input, item_embedding_input,
                                              timediffs=user_timediffs_tensor, features=feature_tensor,
                                              select='user_update')
        item_embedding_output = model.forward(args,user_embedding_input, item_embedding_input,
        ...
        user_embeddings_timeseries[j, :] = user_embedding_output.squeeze(0)[:args.embedding_dim]
        item_embeddings_timeseries[j, :] = item_embedding_output.squeeze(0)
        #local_embeddings = compute_NN()

        user_current_timestamp[userid] = timestamp_sequence[j]
        user_current_transactionids[:, userid] += 1
        user_current_transactionids[userid, :] = 0
        local_embedding = model.compute_NN(itemid_tensor, userid_tensor,local_embedding, item_user)
        item_user[itemid_tensor, userid_tensor]=1
